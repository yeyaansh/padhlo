  ...
  return (
    <div className="bg-white rounded-xl sketch-border-1 p-6">
      <div className="flex flex-col sm:flex-row items-center gap-6">
        ...
        />
        <div>
          <h1 className="text-4xl font-['Permanent_Marker'] text-gray-800">{user.username}</h1>
          <p className="text-gray-500">{user.joinDate}</p>
        </div>
      </div>
      <div className="mt-6 grid sm:grid-cols-3 gap-4">
        <StatCard label="Problems Solved" value={user.stats.solved} icon="✅" />
        <StatCard label="Total Attempts" value={user.stats.attempted} icon="✏️" />
        <StatCard label="Playlists Created" value={user.stats.playlists} icon="🎵" />
      </div>
    </div>
  );
}